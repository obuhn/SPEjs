 ...
 * @license GNU-GPL 
 *
 * @author obuhn https://github.com/obuhn
 * @updated 2018-04-11
 * 
 *
 *
...
module.exports = function (babel) {
	var t = babel.types;
	var solver = { name: "z3", path: "/usr/bin/z3", tmpPath: "/home/obuhn/Desktop/task6/tmp" };
	var ws = new Set();
	var env = {
			...
			IfStatement: {
				...
				exit(path) {
					if (path.node.test.type == "BooleanLiteral") {
						...
						Identifier(path) {
							if (env[path.node.name]!= null && env[path.node.name].constraint!= null) {
				...
				exit(path) {
						...
						if (path.node.init == null) {
							...
							if (env[path.node.id.name] != null && env[path.node.id.name].value != null){
								...
								env[path.node.id.name].type ="Int";
							}
							else{
								env[path.node.id.name] = { value: path.node.init.value };
								env[path.node.id.name].type ="Int";
